...
class Init {
	static injectCommand(vorpal) {
		vorpal
		  ...
		  .action(function(args, callback) {
		    ...
		    if(fs.existsSync("./apiproxy")) {
		    	...
		    	if(title == null) {
		    		console.log(global.chalk.red("Structure error in ApiProxy folder"));
		    		callback();
		    	} else {
		    		auxVorpal.delimiter(global.chalk.green(title + "$"));
		    		callback();
		    	}
		    } else {
		...
		vorpal
		.command('init rewrite', 'Rewrite all xml files in apiproxy')
		.action(function(args, callback) {
			...
			if(fs.existsSync("./apiproxy")) {
		  	...
		  	auxVorpal.log(global.chalk.yellow("Can't rewrite, apiproxy folder doesn't exists!!!"));
		  }    
		});
	...
	static parseSwagger(file, callback) {
		SwaggerParser
			...
			.catch(function(err) {
				console.error(global.chalk.red('Onoes! The API is invalid. ' + err.message));
				callback(null);
			});
	}
