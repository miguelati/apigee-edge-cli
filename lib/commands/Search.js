...
class Search {
	static injectCommand(vorpal) {
		...
		vorpal
		.command('search policyInFlow <query>', 'Information of actual apiproxy')
		    	...
		    	if(policies.indexOf(args.query) == -1) {
		    		...
		    		for(let index in proxyEndpoints) {
		    			...
		    			for(let indexFlow in flows) {
		    				...
		    				let response = proxyEndpointHelper.getFlowRequestStepNames(flows[indexFlow]);
		    		...
		    		for(let index in targetEndpoints) {
		    			...
		    			for(let indexFlow in flows) {
		    				...
		    				for(let indexRequest in request) {
		    					if(request[indexRequest] === args.query) toFind.push(`ProxyEndpoint ${targetEndpointHelper.getName()} ${arrow} Flow ${flows[indexFlow]} ${arrow} Request`);
		    				}
		    				let response = targetEndpointHelper.getFlowRequestStepNames(flows[indexFlow]);
		    				for(let indexResponse in response) {
		    					if(response[indexResponse] === args.query) toFind.push(`ProxyEndpoint ${targetEndpointHelper.getName()} ${arrow} Flow ${flows[indexFlow]} ${arrow} Response`);
		    				}
		    			}
		    		}
		    		//console.log(toFind);
