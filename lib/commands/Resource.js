...
class Resource {
	static injectCommand(vorpal) {
		...
		.action(async (args, callback) => {
			try {
				if(APIProxyHelper.existsResource(`node://${args.resourceName}`)) await Command.openFile(`./${global.actualRevision}/apiproxy/resources/node/${args.resourceName}`);
		...
		.action(async (args, callback) => {
			try {
				if(APIProxyHelper.existsResource(`jsc://${args.resourceName}`)) await Command.openFile(`./${global.actualRevision}/apiproxy/resources/jsc/${args.resourceName}`);
				else global.output.error(`Resource ${args.resourceName} doesn't exists!!`);
			} catch(e) {
				global.output.error();
			} finally {
