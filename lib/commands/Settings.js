...
let settings = [
	...
	//'production.rules.repoUrl', 
	//'production.rules.testing' 
];
...
class Settings {
	...
	static async verifyProductionRulesEnabled() {
		let prompt = new Prompt();
		prompt.confirm('rulesEnabled', 'Do you want to enable Production Rules?', {default: true});
		let result = await prompt.show();
		if(global.prefs.production === undefined) global.prefs.production = {};
		if(global.prefs.production.rules === undefined) global.prefs.production.rules = {};
		global.prefs.production.rules.enabled = result.rulesEnabled;
	}

	static async verifyProductionRulesDescription() {
		let prompt = new Prompt();
		prompt.confirm('description', 'Process to modify Rule Description:', {default: true});
		let result = await prompt.show();
		if(global.prefs.production === undefined) global.prefs.production = {};
		if(global.prefs.production.rules === undefined) global.prefs.production.rules = {};
		global.prefs.production.rules.description = result.description;
	}
}
