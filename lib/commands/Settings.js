	...
	static injectCommand(vorpal) {
		...
		.action(function(args, callback) {
			...
			if(global.prefs.apigee) {
				rows.push(["apigee.username", global.prefs.apigee.username]);
				rows.push(["apigee.password", "***************"]);
				rows.push(["apigee.organization", global.prefs.apigee.username]);
			} else {
				rows.push(["apigee.username", ""]);
				rows.push(["apigee.password", ""]);
				rows.push(["apigee.organization", ""]);
			}
	    	
	    	if(global.prefs.editor) {
	    		rows.push(["editor", global.prefs.editor]);	
	    	} else {
	    		rows.push(["editor", ""]);
	    	}
		...
		.autocomplete({
				data: function () {
					return ['apigee.username', 'apigee.password', 'apigee.organization', 'editor'];
				}
		})
		.action(function(args, callback) {
			if(args.key === 'editor') {
