...
class Settings {
	...
	static verifyLiveValidation(vorpal, callback) {
		vorpal.prompt({type: 'confirm', name: 'validation', message: 'You want live validation?', default: true}, (result) => {
	...
	static verifyLiveUpload(vorpal, callback) {
		vorpal.prompt({type: 'confirm', name: 'upload', message: 'You want live upload?', default: false}, (result) => {
			if(global.prefs.live === undefined) global.prefs.live = {};
			global.prefs.live.upload = result.upload;
			Command.getApigeeData(vorpal, () => {
				callback();
			});
		});
	}

	static verifyTemplate(vorpal, callback) {
		let validation = (value) => value === "" || fs.existsSync(fs.absolute(value));
		
		vorpal.prompt({type: 'input', name: 'template', message: 'Enter template\'s path (empty is cancel) :', validate: validation}, (result) => {
