	...
	static injectCommand(vorpal) {
		...
		.action(async (args, callback) => {
			...
			try {
				countdown.start()
				let info = APIProxyHelper.getInfo();
				let revisions = await await ApiProxyRest.getRevisions({apiproxyName: info.name}, global.prefs.apigee);
				countdown.stop();
				...
				if(args.options.force) await ApiProxyRest.setUndeploy({name: info.name, revision: result.revision, environment: result.environment}, global.prefs.apigee);
		...
		.action(async (args, callback) => {
			...
			try {
				...
				global.output.success(`${info.name}'s revision #${revision} was deployed in ${result.environment}`);
