...
class ApiProxy {
	static injectCommand(vorpal) {
		...
		.action(async (args, callback) => {
			...
			try {
				...
				if(rows.length == 0 ){
					rows.push(["There are no deployed reviews"]);
				}
		...
		.action(async (args, callback) => {
			...
			try {
				...
				let resp = await ApiProxyRest.setDeploy({name: info.name, revision: result.revision, environment: result.environment}, global.prefs.apigee);
				countdown.stop();
				
				if(resp.message === undefined){
					global.output.success(`${info.name}'s revision #${result.revision} was deployed in ${result.environment}`);
				}else{
					global.output.success(global.chalk.red(`Error: ${resp.message}`));
				}
		...
		.action(async (args, callback) => {
			...
			try {
				...
				countdown.stop();
				let prompt = new Prompt();
				prompt.list('environment', 'Select a environment to undeploy', ['prod', 'test']);
				...
				let resp = await ApiProxyRest.setUndeploy({name: info.name, revision: revision, environment: result.environment}, global.prefs.apigee);
				countdown.stop();
				if(resp.message === undefined){
			...
			try {
				...
				if(fs.existsSync(`./${args.revisionNumber}/apiproxy`)) {
					...
					if(fs.existsSync(`./${args.revisionNumber}/apiproxy/resources/node/`) && global.prefs.node.zip) {
						...
						APIProxyHelper.generateResources(args.revisionNumber);
		...
		.action(async (args, callback) => {
			...
			try {
				...
				if(!_.isEmpty(args.options.description)) apiproxyHelper.changeDescription(args.options.description);
	...
	static async existSwagger(vorpal) {
		let swaggers = fs.listSync("./", ["yml", "yaml", "json"]);
