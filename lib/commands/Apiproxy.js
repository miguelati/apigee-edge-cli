...
class ApiProxy {
	static injectCommand(vorpal) {
		...
		vorpal
		.command('apiproxy change [revisionNumber]')
		.action((args, callback) => {
			if(_.isUndefined(args.revisionNumber)) {
				...
				if(revisions.indexOf(`${args.revisionNumber}`) !== -1) {
					Enviroment.changeRevision(args.revisionNumber, vorpal);
					callback();		
				} else {
					global.output.error(`Revision ${args.revisionNumber} wasn't downloaded`);
					callback();
				}
			}
		});
