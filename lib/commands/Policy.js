...
class Policy {
	static injectCommand(vorpal) {
		...
		.action(async (args, callback) => {
			try {
				if(APIProxyHelper.existsPolicy(args.policyName)) await Command.openFile(this, `./${global.actualRevision}/apiproxy/policies/${args.policyName}.xml`);
		...
		.action(async (args, callback) => {
			try {
				if(APIProxyHelper.existsPolicy(args.policyName)) {
					...
					let vorpal = this;
					let result = await Command.getListPolicies(vorpal);
					...
					if(result.policyName == 'Access Control') {
						...
						await Command.openFile(vorpal, `./${global.actualRevision}/apiproxy/policies/${args.policyName}.xml`);
					} else {
