...
class ApiProxy {
	...
	createFoldersStructure() {
		var dirs = ["./apiproxy/policies/", "./apiproxy/proxies/", "./apiproxy/resources/jsc", "./apiproxy/targets"];

		for (var i = 0; i < dirs.length; i++) {
			if(!fs.existsSync(dirs[i])) {
				fs.makeTreeSync(dirs[i]);
				console.log(global.chalk.yellow("Create " + dirs[i]));
			}
		}
	}
	...
	createTargetEndpoint(name = "default") {
		...
		targetEndpoint.createEmptyWithHTTPTargetConnection("http://example.com");
	...
	static writeFile(path, content) {
		if(fs.existsSync(path)) {
			...
			console.log(global.chalk.magenta("Updated " + path));
		} else {
			fs.writeFileSync(path, content, { encoding: 'utf8'});
			console.log(global.chalk.yellow("Created " + path));
		}
	}
}
