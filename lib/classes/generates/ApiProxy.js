...
class ApiProxy {
	...
	createTargetEndpoint(name = "default") {
		...
		this.writeFile("./apiproxy/targets/"+targetEndpoint.name+".xml", targetEndpoint.toXml());
	}
	...
	createApiProxyFile(swagger) {
		...
		this.writeFile("./apiproxy/"+apiProxy.name+".xml", apiProxy.toXml());
	}
	...
	createProxyEndpoint(swagger) {
		...
		this.writeFile("./apiproxy/proxies/"+proxyEndpoint.name+".xml", proxyEndpoint.toXml());
	}
	...
	createOAuth2Policy(name="VerifyAccessTokenPolicy") {
		...
		this.writeFile("./apiproxy/policies/" + _.kebabCase(name) +".xml", oAuthV2.toXml());
	}
	...
	createExtractVariable(name, path, verb, parameters) {
		...
		this.writeFile("./apiproxy/policies/"+ _.kebabCase(name) +".xml", extractVariable.toXml());
