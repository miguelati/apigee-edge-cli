...
class Flow {
	...
	showTable() {
		...
		for(let index in this._flows) table.push([Number(index) + 1, this._flows[index]]);

    	this._buffer += "\n\n" + table.toString();
	}

	checkGraphWithNames(callback) {
	...
	drawNormal(addArrow = false) {
		...
		for(let index in this._flows) {
			if(this._type == 'response') {
				...
				totalPosition = totalPosition - (this._flows[index].length + (this._boxPadding * 2)) - this._boxSpacing;
			}
	...
	addPolicy(index, onlyNumber, position) {
		if(onlyNumber) {
			let box = Box( this._minBoxSize + "x1", Couleurs(Number(index) + 1, "#c0392b"))
			...
			let width = this._flows[index].length + (this._boxPadding * 2);
			let box = Box( width + "x1", Couleurs(this._flows[index], "#c0392b"))
			this._buffer = Overlap({
				    who: this._buffer
				  , with: box
				  , where: {
