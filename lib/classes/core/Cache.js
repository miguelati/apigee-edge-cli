...
class Cache {
	...
	constructor(path) {
		...
		this.loadProxys();
		this.loadTargets();
		//console.log(global.localStorage.getItem("apiproxy.name"));
	}
	...
	loadProxy(file) {
		...
		this.setSeps('proxy.'+ json.ProxyEndpoint["@name"] +'.preflow.request', json.ProxyEndpoint.PreFlow.Request);
		this.setSeps('proxy.'+ json.ProxyEndpoint["@name"] +'.preflow.response', json.ProxyEndpoint.PreFlow.Response);

		// PostFlow
		this.setValue('proxy.'+ json.ProxyEndpoint["@name"] +'.postflow.name', json.ProxyEndpoint.PostFlow["@name"]);
		this.setSeps('proxy.'+ json.ProxyEndpoint["@name"] +'.postflow.request', json.ProxyEndpoint.PostFlow.Request);
		this.setSeps('proxy.'+ json.ProxyEndpoint["@name"] +'.postflow.response', json.ProxyEndpoint.PostFlow.Response);
	...
	loadTarget(file) {
		...
		this.setSeps('target.'+ json.TargetEndpoint["@name"] +'.preflow.request', json.TargetEndpoint.PreFlow.Request);
		this.setSeps('target.'+ json.TargetEndpoint["@name"] +'.preflow.response', json.TargetEndpoint.PreFlow.Response);
		
		// PostFlow
		this.setValue('target.'+ json.TargetEndpoint["@name"] +'.postflow.name', json.TargetEndpoint.PostFlow["@name"]);
		this.setSeps('target.'+ json.TargetEndpoint["@name"] +'.postflow.request', json.TargetEndpoint.PostFlow.Request);
		this.setSeps('target.'+ json.TargetEndpoint["@name"] +'.postflow.response', json.TargetEndpoint.PostFlow.Response);
	...
	setMultiFlows(key, flows) {
		if(_.isNil(flows)) {
			...
			for(let index in flows.Flow) {
				this.setFlow(key + "." + flows.Flow[index]['@name'], flows.Flow[index]);
				nameFlows.push(flows.Flow[index]['@name']);
			}
			this.setValue(key + ".nameFlows", nameFlows, 'object');
		} else {
			this.setFlow(key + "." + flows.Flow['@name'], flows.Flow);
		}
	}

	setFlow(key, content) {
		this.setSeps(key + '.request', content.Request);
		this.setSeps(key + '.response', content.Response);
	}
