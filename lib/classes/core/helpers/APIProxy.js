...
class APIProxy {
	...
	generateResources(revision = null) {
		this.clean('resource');
		if(revision === null) revision = global.actualRevision;
		if(fs.existsSync(`./${revision}/apiproxy/resources/jsc`)) {
			let resourcesFiles = fs.listTreeSync(`./${revision}/apiproxy/resources/jsc/`);
			for(let index in resourcesFiles) {
				if(!fs.isDirectorySync(resourcesFiles[index])) {
					let resource = `jsc://${resourcesFiles[index].replace(`${revision}/apiproxy/resources/jsc/`, "")}`;
					this.add('resource', resource);
				}
			}
		}

		if(fs.existsSync(`./${revision}/apiproxy/resources/node`)) {
			let resourcesFiles = fs.listTreeSync(`./${revision}/apiproxy/resources/node`);
			for(let index in resourcesFiles) {
				if(!fs.isDirectorySync(resourcesFiles[index])) {
					let resource = `node://${resourcesFiles[index].replace(`${revision}/apiproxy/resources/node/`, "")}`;
					this.add('resource', resource);
				}
			}
		}
	...
	changeDescription(description) {
		_.set(this._json, 'APIProxy.Description', description);
		this.save();
	}
	...
	static generateResources(revision = null) {
		let me = new APIProxy();
		return me.generateResources(revision);
	}
	...
	save(revision = null) {
		...
		fs.writeFileSync(`./${revision}/apiproxy/${this._json.APIProxy["@name"]}.xml`, xml, { encoding: 'utf8'});
	}
	...
	remove(type, name) {
		//console.log(this._paths[type]);
		if(_.has(this._json, this._paths[type])) {
			///console.log(`${this._paths[type]}.${this._tagElement[type]}`);
			let object = _.get(this._json, `${this._paths[type]}.${this._tagElement[type]}`);
			//console.log(JSON.stringify(this.removeObject(object, name)));
	...
	static existsResource(name) {
		try {
			...
			//let me = new APIProxy();
			let path = name.split("://");
			return fs.existsSync(`./${global.actualRevision}/apiproxy/resources/${path[0]}/${path[1]}`);//me.exists('resource', name);	
