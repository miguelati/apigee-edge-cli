...
class APIProxy {
	...
	add(type, name) {
		let path = this._paths[type];
		if(dotty.exists(json, path)) {
			let object = dotty.get(json, path);
			dotty.put(this._json, path, this.addToObject(object, name));
			this.save();
		}
	}
	...
	exists(type, name) {
		...
		if(dotty.exists(this._json, path)) {
			return this.verifyIfExists(dotty.get(this._json, path), name);
		} else {
			return false;
		}
	}
	...
	get(type) {
		...
		if(dotty.exists(this._json, path)) {
			return this.getObject(dotty.get(this._json, path));
		} else {
			return [];
		}
	}
	...
	remove(type, name) {
		let path = this._paths[type];
		if(dotty.exists(this._json, path)) {
			let object = dotty.get(this._json, path);
			dotty.put(this._json, path, this.removeObject(object, name));
			this.save();
		}
	}
