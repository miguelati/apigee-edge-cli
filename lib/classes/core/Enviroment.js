...
class Enviroment {
	static getAllRevisionsDownloaded() {
		return fs.listSync('./').filter(item => fs.isDirectorySync(item) && !item.startsWith('.') && /^[0-9]+$/.test(item) && fs.existsSync(`./${item}/apiproxy/`) && fs.isDirectorySync(`./${item}/apiproxy/`)).sort();
	}
	...
	static init(vorpal) {
		...
		let defaults = {apigee: {}, live: {validation: true, upload: false}, node: {zip: false}};
