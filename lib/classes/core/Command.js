...
class Command {
	...
	static async openFile(vorpal, fileName) {
		try {
			...
			await Command.openInEditor(fileName);
	...
	static async getApigeeData(vorpal) {
		let validation = (value) => value != '';
		let prompt = new Prompt(vorpal);
	...
	static analizeVerbInCondition(condition) {
		...
		if(_.isEmpty(condition)) {
			...
			return global.chalk.white.bgRed.bold(_.pad("ALL", 12));
		} else {
	...
	static async getListPolicies(vorpal, callback) {
