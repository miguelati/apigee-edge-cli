...
class ApiProxyRest {
	...
	static url(organization) {
		return format("https://api.enterprise.apigee.com/v1/organizations/{organization}", {organization: organization});
	}
	...
	static update(params, auth, callback) {
		let options = {
			method: 'POST',
			uri: format(ApiProxyRest.url(auth.organization) + "/apis/{name}/revisions/{revision}", {name: params.name, revisionNumber: params.revision}),
			headers: {
