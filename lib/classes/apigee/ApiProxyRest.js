...
class ApiProxyRest {
	...
	static delete(params, auth, callback) {
		let options = {
            method: 'DELETE',
            uri: `${ApiProxyRest.url(auth.organization)}/apis/${params.name}`,
			auth: ApiProxyRest.getAuth(auth.username, auth.password)
		};
		request(options, callback);
	}
	...
	static deploy(params, auth, callback) {
		let options = {
            method: 'POST',
            uri: `${ApiProxyRest.url(auth.organization)}/enviroments/${params.env}/apis/${params.name}/revisions/${params.revision}/deployments`,
			headers: {
