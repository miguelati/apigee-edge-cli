...
require('fs').readdirSync(__dirname + '/flows/').forEach(function(file) {
  if (file.match(/\.js$/) !== null && file !== 'index.js') {
    var name = file.replace('.js', '');
    classes[name] = require(__dirname + '/flows/' + file);
  }
});

require('fs').readdirSync(__dirname + '/policies/').forEach(function(file) {
  if (file.match(/\.js$/) !== null && file !== 'index.js') {
    var name = file.replace('.js', '');
    classes[name] = require(__dirname + '/policies/' + file);
  }
});

class TagFactory {
    constructor (tagName, opts) {
        if (tagName in classes) {
            let tag = new classes[tagName](opts);
